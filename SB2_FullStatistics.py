...
import numpy as np
from SB2_PosteriorMode import SB2_PosteriorMode
from SB2_Sigmoid import SB2_Sigmoid

def SB2_FullStatistics(LIKELIHOOD, BASIS, PHI, Targets, Used, Alpha, beta, Mu, BASIS_PHI, BASIS_Targets, OPTIONS):
    ...
    if LIKELIHOOD['InUse'] == LIKELIHOOD['Gaussian']:
        ...
        U           = np.linalg.cholesky(PHI.T * PHI * beta + np.diagflat(Alpha))
        Ui          = np.linalg.inv(U).T
        SIGMA       = Ui * Ui.T

        # Posterior mean Mu
        Mu          = (SIGMA * (PHI.T * Targets)) * beta
        ...
        [Mu, U, beta, dataLikely, a] = SB2_PosteriorMode(LIKELIHOOD,PHI,Targets,Alpha,Mu, MAX_POSTMODE_ITS,OPTIONS)
    ...
    logdetHOver2    = np.sum(np.log(U.diagonal()))
    logML            = dataLikely - (np.power(Mu,2)).T*Alpha/2 + np.sum(np.log(Alpha))/2 - logdetHOver2
    
    # Well-determinedness factors
    DiagC    = np.sum(np.power(Ui,2),1)
    Gamma    = 1 - np.multiply(Alpha,DiagC)
    ...
    if LIKELIHOOD['InUse'] == LIKELIHOOD['Gaussian']:
        ...
        S_in            = beta - np.sum(np.power(betaBASIS_PHI*Ui, 2), 1)
        ...
        betaBASIS_PHI   = BASIS.T*(np.multiply(PHI, beta*np.ones((1,M))))
        S_in            = (beta.T * np.power(BASIS,2)).T - np.sum(np.power(betaBASIS_PHI*Ui,2), 1)
        Q_in            = BASIS.T*e
    ...
    S_out[Used]    = np.divide(np.multiply(Alpha, S_in[Used]) , (Alpha - S_in[Used]))
    Q_out[Used]    = np.divide(np.multiply(Alpha, Q_in[Used]) , (Alpha - S_in[Used]))
